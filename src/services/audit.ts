import { fetchChatCompletions } from './aiService'; // Updated import
...
IMPORTANTE: Asegúrate de que el JSON resultante sea válido y pueda ser parseado por JSON.parse().`;
  ...
  try {
    const payload = {
      ...
      // model: import.meta.env.VITE_AZURE_OPENAI_DEPLOYMENT, // Model handled by proxy
      temperature: 0.5,
      max_tokens: 2500,
    ...
    if (data && data.informe_auditoria) {
      parsedResponse = data;
      ...
      try {
        ...
        console.error('Respuesta no válida del modelo (generateAudit):', responseContent);
        throw new Error('La respuesta del modelo no es un JSON válido');
      }
    } else {
      throw new Error("Respuesta inesperada del servicio de IA para generateAudit.");
    }

    // Verifica la estructura del JSON
    if (!parsedResponse.informe_auditoria) {
      console.error('Estructura JSON incorrecta (generateAudit):', parsedResponse);
    ...
    for (const prop of requiredProps) {
      if (!(prop in audit)) {
        console.error(`Falta la propiedad ${prop} en el informe (generateAudit)`);
    ...
    for (const category of requiredCategories) {
      if (!audit.lista_verificacion[category as keyof typeof audit.lista_verificacion]) {
        console.error(`Falta la categoría ${category} en lista_verificacion (generateAudit)`);
    ...
    if (error instanceof Error) {
      ...
      throw new Error('Error desconocido al generar el informe de auditoría');
    }
  }
}
...
6. Las recomendaciones y próximos pasos deben ser específicos y accionables`;
  ...
  try {
    ...
    const data = await fetchChatCompletions(payload);
    let parsedResponse: HallazgoAuditoria;

    if (data && data.informe_hallazgo) {
      parsedResponse = data;
    } else if (data && data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content) {
      const responseContent = data.choices[0].message.content;
      if (!responseContent) throw new Error('No se recibió contenido JSON del servicio de IA');
      try {
        const cleanContent = responseContent.trim().replace(/^```json\s*/, '').replace(/\s*```$/, '').replace(/^\s*\{\s*/, '{').replace(/\s*\}\s*$/, '}');
        parsedResponse = JSON.parse(cleanContent);
      } catch (parseError) {
        console.error('Respuesta no válida del modelo (generateAuditFindings):', responseContent);
        throw new Error('La respuesta del modelo no es un JSON válido');
      }
    } else {
      throw new Error("Respuesta inesperada del servicio de IA para generateAuditFindings.");
    }

    if (!parsedResponse.informe_hallazgo) {
      console.error('Estructura JSON incorrecta (generateAuditFindings):', parsedResponse);
      throw new Error('La respuesta no tiene la estructura correcta');
    }
    ...
    for (const section of requiredSections) {
      if (!(section in informe)) {
        console.error(`Falta la sección ${section} (generateAuditFindings)`);
    ...
    if (!Array.isArray(informe.hallazgos) || informe.hallazgos.length !== (hallazgos || []).length) {
      console.warn(`Número de hallazgos en respuesta (${informe.hallazgos?.length}) no coincide con input (${(hallazgos || []).length}). Se procederá con los hallazgos de la respuesta.`);
      // Potentially adjust logic or throw error if exact match is critical
    }

    const resumen = informe.conclusiones.resumen_hallazgos;
    const hallazgosGenerados = informe.hallazgos || []; // Ensure hallazgosGenerados is an array
    ...
    if (JSON.stringify(resumen) !== JSON.stringify(conteoReal)) {
      console.warn('Corrigiendo el resumen de hallazgos para que coincida con el conteo real (generateAuditFindings)');
    ...
    if (!idsCorrectos) {
      console.warn('Corrigiendo IDs de hallazgos (generateAuditFindings)');
    ...
    if (error instanceof Error) {
      ...
      throw new Error('Error desconocido al generar el informe de hallazgos');
    }
  }
}
